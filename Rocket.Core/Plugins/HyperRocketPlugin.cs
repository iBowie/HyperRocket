...
{
    public class HyperRocketPlugin<RocketPluginConfiguration> : HyperRocketPlugin, IRocketPlugin<RocketPluginConfiguration> where RocketPluginConfiguration : class, IRocketPluginConfiguration
    {
        public IAsset<RocketPluginConfiguration> Configuration { get; }

        public HyperRocketPlugin() : base()
        {
            string configurationFile = Path.Combine(Directory, string.Format(Core.Environment.HyperPluginConfigurationFileTemplate, Name));
            ...
            {
                Configuration = new JSONFileAsset<RocketPluginConfiguration>(configurationFile, serializerSettings: new Newtonsoft.Json.JsonSerializerSettings() { TypeNameHandling = Newtonsoft.Json.TypeNameHandling.Auto });
            }
        }
    }

    public class HyperRocketPlugin : MonoBehaviour, IRocketPlugin
    {
        ...
        private JSONFileAsset<TranslationList> translations;
        ...
        public HyperRocketPlugin()
        {
            ...
            {
                translations = new JSONFileAsset<TranslationList>(Path.Combine(Directory, String.Format(Environment.HyperPluginTranslationFileTemplate, Name, R.Settings.Instance.LanguageCode)), defaultInstance: DefaultTranslations);
